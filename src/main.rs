...
impl Animal {
    ...
    fn move_animal(&mut self, dx: i32, dy: i32, game_state: &GameState) -> bool {
        let new_x = self.x + dx;
        let new_y = self.y + dy;
...
impl Sheep {
    ...
    fn flee(&mut self, game_state: &GameState) -> Option<String> {
        ...
        if magnitude != 0.0 {
            ...
            if self.animal.move_animal(normalized_dx, normalized_dy, game_state) {
                self.energy -= 10;
                ...
                if animal.animal_type == "wolf" {
                    ...
                    if !targets.is_empty() {
                        ...
                        if distance <= WOLF_ATTACK_RANGE as f64 {
                            ...
                            if animal.move_animal(dx, dy, &self.game_state) {
